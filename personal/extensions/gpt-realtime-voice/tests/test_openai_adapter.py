...
def test_session_disables_model_owned_replies() -> None:
    request = OpenAIRealtimeAdapter(VoiceRuntimeConfig()).session_request()
...
async def test_browser_payload_contains_only_short_lived_session_data() -> None:
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
        ...
        payload = await adapter.create_browser_session("test-secret", "safe-id")
...
async def test_upstream_errors_do_not_echo_response_body() -> None:
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
        ...
        with pytest.raises(RealtimeUpstreamError) as exc_info:
            await adapter.create_browser_session("bad-key", "safe-id")
